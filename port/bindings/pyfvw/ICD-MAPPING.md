...
| ICD / COM surface | pyfvw | Semantic deltas |
|---|---|---|
| MDM (Map Data Manager) `tblDataSources` / `tblMapSeries` / `tblCoverage` | `catalog.Catalog` (SQLite; `add_data_source` / `scan` / `series` / `select_by_geo_rect` / `best_series_for_scale` / `remove_data_source`) | Schema mirrors the three MDM tables, **including tblMapSeries's identity**: a series is (format, series_key, scale, scale_units), as `CCoverageCache::GetMapSeriesIdentity` looks one up by `SelectByScale(scale, units, series)`. `SeriesRow.display_name` is the `FORMAT_SERIES_SCALE` label ("Color 1 meter") and is the unique handle; `series_key` is not. `scan` drives the FvKit format registry (not per-server GenerateCoverage). `db_path` defaults to `":memory:"`. Opening a schema-1 file rebuilds it (`Catalog.needs_rescan`). |
...
## Slice 11 (2026-08-13): the application layer — `pyfvw.app` (A6)

| ICD / COM surface | pyfvw | Semantic deltas |
|---|---|---|
| `COverlayTypeDescriptorList` + `IFvOverlayTypeDescriptor` (the overlay-type registry) | `app.OverlayTypeDesc`, `app.OverlayTypeRegistry`, `app.register_builtin_types` | Identity is a **string** TypeId (`"fv.route"`), not a GUID — diffable and readable in a settings file, which matters because the display order and the restored session are written to `Settings`. The factory is a plain callable (`IFvOverlayFactory` had one method and no state). `file=None` **is** the static/file distinction: no `FileTypeDesc` ⇒ at most one instance, toggled. Left behind: backing-store enum, help ids, ribbon icon resources, COM custom initializers, `HCURSOR`. |
| `C_contour_ovl` + `CContourTile` + `CContourLists` (the Contour Lines overlay) | `overlay.ContourOverlay`, and `fvkit/geo/terrain_contour.h` under it | Marching squares replaces the multimap-of-cell-ids assembly; a void post is NaN and stops the cell rather than being traced as -32767 m. `DataSource` (a DTED level picked by hand) is gone — the elevation source already prefers the finest cell it has. `ThinningLevel`, which the original stored and never read, became `thinning_px`, a Douglas-Peucker tolerance that runs before the smoothing the original never had. |
| `IFvOverlayPersistence`, `OverlayContextMenu_Interface`, `test_select`/`test_snap_to`, `OverlayUIEventRoutingOverrides` | methods you define on `overlay.Overlay` | FalconView discovered these with `dynamic_cast`; here a capability is **a method you defined** (`file_open`, `hit_test_point`, `menu_items`, `snap_to_point`, `wants_direct_routing`, `enter_edit_focus`…). Same optionality, no registration, and safe across the trampoline, which a cross-cast is not. Persistence STATE is on the overlay (`.dirty`, `.file_spec`, `.has_been_saved`, `.read_only`); a failure is reported by **raising**, and an exception carrying `.code` keeps it. |
| `AfxMessageBox` / `CFileDialog` / `snptodlg` / every inline modal | `app.AppShell` (subclass it) | Rule R1: the core states what needs DECIDING and never decides how to ask. Five decisions (`ask_save`, `choose_files_to_open`, `choose_save_spec`, `choose_from_list`, `confirm_revert`) and six presentation calls. Cancel is expressed in the shape of the answer — `[]`, `""`, `None` — because "chose nothing" and "cancelled" are one outcome for every caller. |
| `C_ovl_mgr::{open, create, close, save, save_as, save_all, toggle_static_overlay, OpenFileOverlay, exit}`, `save_overlay_configuration` | `app.OverlaySession` | Every verb returns a **`FlowResult`**, never a bool: `CANCELED` is the user's answer and propagates (one cancel in one save prompt aborts the whole `close_all`, and with it the exit), `FAILED` has already reached `report_error`. Open dedup is `(TypeId, file spec)`. `save_configuration`/`restore_configuration` populate `Settings` **in memory** — the INI is the user's file and the port never rewrites it (rule S1). |
| `C_ovl_mgr::{set_mode, toggle_editor, SwitchToEditor, make_mode_match_current_overlay}` + `IFvOverlayEditor` + `IFvOverlayLimitUserInterface` | `app.EditorManager`, and an editor is any object with `activate()`/`deactivate()` | One editor instance per **TYPE** (the editor is the tool state), cached, so tool state survives leaving and re-entering. `set_mode` makes the current overlay match the mode; "the mode follows the current overlay" is **observed** through a stack observer, so it holds however the change was made. `ui_constraints()` is reported, never enforced — the core owns no controls. |
| `test_select`/`selected` (first-hit-wins veto in stack order) | `app.PickSession` | Reorganised rather than mirrored: picking is an **aggregation** over the `hit_test_point` capability, ranked by a stated `PickPolicy` (`TOP_MOST` / `NEAREST` / `ASK_WHEN_AMBIGUOUS`). Who is asked is the draw order reversed, so a tap agrees with the screen. A hover notifies the shell **only on a change**. Picking does not replace routing: route the event first, resolve here only when nothing consumed it. |
| `OverlayStackChangedObserver_Interface`, and `C_ovl_mgr`'s z-order/current/declutter/capture | `app.StackObserver` + the A2 surface on `overlay.OverlayManager` | `set_type_registry` (insertion by display order; without it `add` is a plain append), `current`/`make_current`, `move_above`/`move_below`/`move_to_bottom`/`reorder` (a total permutation, rejected whole if it is not one), `first_of_type`/`of_type`/`find_by_file_spec`, `declutter`, `capture_mouse`, `draw_order`. |
| *(no COM predecessor)* — a document of geographic points | `overlay.PointOverlay`, `overlay.MapPoint` | The first C++ **file** overlay: a `.fvpoints` **SQLite** document (`points` + `meta`), drawn as geometric shapes, persistent + pickable + with a context menu. `HitItem.feature` is the row's own id, so a selection survives a redraw. `PointOverlay.write_sample_file(path)` writes an arbitrary starter set. |
...
- **FeatureStore** (the feature half of L5; only the raster TilePack store is
  bound).
- **An editor for `fv.points`** — the point overlay is a document and a pick
  target, and every mutator is bound, but no editor makes the two gestures
  (click-to-place, drag-to-move) reachable from a UI.

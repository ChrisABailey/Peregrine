...
| `port/pippin-guidance-plan.md` | Pippin's turn guidance (GD1–GD4: derived maneuvers, the approach state machine, the on-screen arrow and countdown) and screen-off tracking (BG1–BG5: scene-phase gating, the background location capability, the accuracy policy in the dark, orphaned rides, notifications that mirror to a watch) | **NOT STARTED — deferred by Chris 2026-09-04 until Pippin v1 is approved.** Order is fixed: GD before BG. The survey behind it is in the plan's first section; do not re-explore for it |
...
```sh
cmake -B build && cmake --build build -j && ctest --test-dir build   # 1962 as of 2026-09-02, ALL GREEN.
...
`geo/contour.h`, which is next door and is about geographic CURVES) and
`overlay/contour_overlay.h` (`fv.contour`, the ported Contour Lines overlay).
...
- **`OsmNameIndex`'s eight tests fail under `ctest -j`, and a different subset each run** (seen
  ...
  suite; nothing about the name index itself is wrong.

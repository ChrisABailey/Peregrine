...
typedef struct {
  ...
  JSAMPARRAY12 colormap;		/* GIF colormap (converted to my format) */
...
METHODDEF (unsigned int) get_pixel_rows
	JPP((j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo));
METHODDEF (unsigned int) load_interlaced_image
	JPP((j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo));
METHODDEF (unsigned int) get_interlaced_row
	JPP((j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo));
...
void
ReadColorMap (gif_source_ptr sinfo, int cmaplen, JSAMPARRAY12 cmap)
/* Read a GIF colormap */
{
  int i;
...
#define UPSCALE(x)  (x)
#else
#define UPSCALE(x)  ((x) << (BITS_IN_JSAMPLE12-8))
#endif
    cmap[CM_RED][i]   = (JSAMPLE12) UPSCALE(ReadByte(sinfo));
    cmap[CM_GREEN][i] = (JSAMPLE12) UPSCALE(ReadByte(sinfo));
    cmap[CM_BLUE][i]  = (JSAMPLE12) UPSCALE(ReadByte(sinfo));
  }
}
...
METHODDEF (void)
start_input_gif (j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo)
{
  ...
  cinfo->data_precision = BITS_IN_JSAMPLE12; /* we always rescale data to this */
...
get_pixel_rows (j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo)
{
  gif_source_ptr source = (gif_source_ptr) sinfo;
  register int c;
...
METHODDEF (unsigned int)
load_interlaced_image (j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo)
{
  gif_source_ptr source = (gif_source_ptr) sinfo;
  JSAMPARRAY12 image_ptr;
  register JSAMPROW12 sptr;
  register JDIMENSION col;
  JDIMENSION row;
  cd_progress_ptr progress = (cd_progress_ptr) cinfo->progress;
  ...
  for (row = 0; row < cinfo->image_height; row++) {
    ...
    image_ptr = (*cinfo->mem->access_virt_sarray)
      ((j_common_ptr12) cinfo, source->interlaced_image,
       row, (JDIMENSION) 1, TRUE);
    sptr = image_ptr[0];
    for (col = cinfo->image_width; col > 0; col--) {
      *sptr++ = (JSAMPLE12) LZWReadByte(source);
    }
  }
  if (progress != NULL)
...
METHODDEF (unsigned int)
get_interlaced_row (j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo)
{
  gif_source_ptr source = (gif_source_ptr) sinfo;
  JSAMPARRAY12 image_ptr;
  register int c;
  register JSAMPROW12 sptr, ptr;
  register JDIMENSION col;
  register JSAMPARRAY12 colormap = source->colormap;
  JDIMENSION irow;
...
METHODDEF (void)
finish_input_gif (j_compress_ptr12 cinfo, cjpeg_source_ptr sinfo)
{
  /* no work */
}
...
cjpeg_source_ptr
jinit_read_gif (j_compress_ptr12 cinfo)
{
